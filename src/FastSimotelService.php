...
class FastSimotelService
{
    public static function run_fast_campaign()
    {
        ...
        */
        if($request_method == 'POST'){
            ...
            $numbers = isset($_POST['numbers']) ? json_decode($_POST['numbers']) : [];
            $file = $_FILES['file']['size'] > 0 ? $_FILES['file'] : "";
            $filename = isset($_POST['filename']) ? $_POST['filename'] : "";
            $campain_name = generateRandomString(10);
            if($file == "" || count($numbers) == 0){
                $result = [
                    "status" => false,
